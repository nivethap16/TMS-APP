document.addEventListener('DOMContentLoaded', () => {
    ...
    if (submitButton) {
        ...
        submitButton.addEventListener('click', async (event) => {
            ...
            const upgradedMemory = document.getElementById('upgraded-memory').value;

            // Validate input data
            if (!changeNumber || !jid || !upgradedMemory) {
                console.error('All fields are required.');
                alert('Please fill in all fields.');
                return;
            }

            // Split upgradedMemory by comma
            const upgradedMemoryArray = upgradedMemory.split(',').map(item => item.trim());
            ...
            const dataToSend = {
                ...
                changeNumber,
                jid,
                upgradedMemory: upgradedMemoryArray,
                selectedCustomer,
                tag: updateInfo.tag
            };
            ...
            try {
                ...
                const response = await fetch('http://localhost:8060/processFormData', {
                    method: 'POST',
                    headers: {
